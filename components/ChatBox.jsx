...
const ChatBox = ({ chat, currentUser, currentChatId }) => {
  ...
  // other member will be an array but just the index 0 it will be the
  // real other member that the chat with him

  const lastMessage =
    chat?.messages?.length > 0 && chat?.messages[chat?.messages.length - 1];

  // check if the current user seen the message by check the seenBy array
        ...
        <div className="flex flex-col gap-1">
          ...
          {lastMessage?.photo ? (
            lastMessage?.sender?._id === currentUser._id ? (
              <p className="text-small-meduim text-grey-3">You sent a photo</p>
            ) : (
              <p
                className={`${
                  seen ? "text-small-meduim text-grey-3" : "text-small-bold"
                } `}
              >
                Received a photo
              </p>
